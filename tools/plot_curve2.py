...
# 4. 设置
OUTPUT_PLOT_PATH = 'similarity_curve_bev_only.png' # 名字改一下，方便区分
STEP_SIZE = 5  # 步长
...
def load_image_tensor(bev_path, range_path, bev_tf, range_tf, device):
    try:
        ...
        # 即便只用BEV，代码结构上还是要读一下Range防止报错，虽然进模型后会被扔掉
        if os.path.exists(range_path):
            img_range = Image.open(range_path).convert('RGB')
        else:
            # 如果没有 range 图，造一个假的黑图防报错
            img_range = Image.new('RGB', (518, 70), (0,0,0))
...
def main():
    ...
    print(f"🚀 启动纯 BEV 对比分析 (Step={STEP_SIZE})... Device: {device}")

    # 1. 加载模型
    print("🏗️  加载模型 (BEV Only Mode)...")
    ...
    # 3. 提取 Anchor 特征
    print(f"⚓ 提取 Anchor: {os.path.basename(ANCHOR_BEV_PATH)}")
    ...
    plt.plot(indices, similarities, label='BEV Only Similarity', color='green', linewidth=1.5)
    ...
    plt.title('Similarity Curve (Ablation: BEV Only, No Fusion)')
