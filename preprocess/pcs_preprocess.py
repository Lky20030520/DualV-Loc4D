...
def main():
    ...
    with open(gps_csv, "w", newline="") as csvfile:
        ...
        writer.writerows(results)
            ...
            # --- main 函数中 ---
    print(f"Loaded {len(times)} pose entries.")
    if len(times) > 0:
        print(f"Pose time range: {times[0]} to {times[-1]}")

    all_files = sorted([f for f in os.listdir(radar_folder) if f.endswith('.pcd')])
    print(f"Found {len(all_files)} total .pcd files in {radar_folder}")
    if len(all_files) > 0:
        print(f"First PCD timestamp: {all_files[0][:-4]}")
