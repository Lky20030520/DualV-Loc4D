...
def _load_dino_feature_lookup(dino_feature_path):
    if not dino_feature_path or not exists(dino_feature_path):
        return {}, None

    payload = torch.load(dino_feature_path, map_location='cpu', weights_only=False)
    if not isinstance(payload, dict):
        return {}, None

    global_vec = payload.get('global_vec', None)
    if global_vec is None:
        return {}, None

    if isinstance(global_vec, torch.Tensor):
        global_vec = global_vec.detach().cpu().float()
    else:
        global_vec = torch.tensor(global_vec, dtype=torch.float32)
    ...
    image_paths = payload.get('image_paths', None)

    if timestamps is not None and len(timestamps) == len(global_vec):
        for idx, ts in enumerate(timestamps):
            key = _to_timestamp_key(ts)
            if key is not None:
                lookup[key] = global_vec[idx]
        return lookup, dim

    if image_paths is not None and len(image_paths) == len(global_vec):
        for idx, p in enumerate(image_paths):
            key = _to_timestamp_key(extract_timestamp(str(p)))
            if key is not None:
                lookup[key] = global_vec[idx]
        return lookup, dim
...
class FusionInferDataset(data.Dataset):
    ...
    def __getitem__(self, index):
            ...
            if self.enable_dino:
                ...
                dino_feat = self._dino_lookup.get(ts_key, None)
                if dino_feat is None:
                    dino_feat = torch.zeros(self.dino_dim, dtype=torch.float32)
                return bev_tensor, range_tensor, dino_feat, index
...
class FusionTrainingDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        def load_and_augment(idx):
            ...
            if len(sample) == 4:
                bev, range_tensor, dino_feat, _ = sample
            else:
                bev, range_tensor, _ = sample
                dino_feat = None
        ...
        # --- 处理 Negatives ---
        n_bevs, n_ranges, n_dinos = [], [], []
        for ni in neg_indices:
            nb, nr, nd = load_and_augment(ni)
            n_bevs.append(nb)
            n_ranges.append(nr)
            if nd is not None:
                n_dinos.append(nd)
        ...
        n_ranges = torch.stack(n_ranges)
        if len(n_dinos) > 0:
            n_dinos = torch.stack(n_dinos)
            return (q_bev, q_range, q_dino), (p_bev, p_range, p_dino), (n_bevs, n_ranges, n_dinos), index
...
def collate_fn_inference(batch):
    ...
    if len(data_tuple) == 4:
        bevs, ranges, dinos, indices = data_tuple
        bevs = torch.stack(bevs, 0)
        ranges = torch.stack(ranges, 0)
        dinos = torch.stack(dinos, 0)
        return (bevs, ranges, dinos), list(indices)
...
def collate_fn(batch):
    ...
    q_bevs = torch.stack([x[0] for x in queries])
    q_ranges = torch.stack([x[1] for x in queries])
    has_dino = len(queries[0]) >= 3
    if has_dino:
        q_dinos = torch.stack([x[2] for x in queries])
    ...
    n_ranges = torch.cat([x[1] for x in negatives])
    if has_dino:
        n_dinos = torch.cat([x[2] for x in negatives])
    
    # 6. 返回打包好的 4 个元素，对应 train_epoch 中的解包顺序
    if has_dino:
        return (q_bevs, q_ranges, q_dinos), (p_bevs, p_ranges, p_dinos), (n_bevs, n_ranges, n_dinos), list(indices)
